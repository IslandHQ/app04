...
export default function CustomDrillPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const qs = await generateDrills(subject, topic, count, [], undefined, instructions);
      if (qs.length > 0) {
        setGeneratedQuestions(qs);
        setSetTitle(`${subject} - ${topic} のオリジナル問題`);
      } else {
        alert('問題の生成に失敗しました。');
      }
    } catch (err: any) {
            ...
            savedSets.map(set => (
              <div key={set.id} className="glass-panel" style={{ padding: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                  ...
                  <button className="btn" style={{ background: 'transparent', color: '#ef4444', padding: '0.5rem' }} onClick={() => handleDelete(set.id)}>
                    <Trash2 size={18} />
                  </button>
                </div>
